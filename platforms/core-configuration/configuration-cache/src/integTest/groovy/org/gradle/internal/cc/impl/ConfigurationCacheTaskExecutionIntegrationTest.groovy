...
class ConfigurationCacheTaskExecutionIntegrationTest extends AbstractConfigurationCacheIntegrationTest {
    ...
    def "task is executed even though its captured state could not be stored"() {
        given:
        settingsKotlinFile ""
        buildKotlinFile """
            plugins { java }
        ...
        then: "the captured state cannot be stored"
        configurationCache.assertStateStoredAndDiscarded {
            hasStoreFailure = false
        ...
        and: "the task is not executed against the unrestorable state"
